...
# MODIFICATION : Le chemin de base pointe maintenant vers un sous-dossier
# situé AU MÊME NIVEAU que le script de la fonction.
# Cela garantit que les PDF sont déployés avec la fonction et sont toujours trouvables.
...
def get_toc_bounds(doc, genus):
    ...
    toc = doc.get_toc(simple=False) # Utiliser simple=False pour avoir les détails de niveau
    if not toc:
    ...
    for i, (level, title, page, _) in enumerate(toc):
        # Le titre dans la TdM est parfois "GENUS Famille" ou juste "GENUS"
        # Utilise une expression régulière pour trouver le genre en début de chaîne comme un mot complet
...
def handler(event, context):
    ...
    try:
        ...
        return {
            'statusCode': 404,
            'body': json.dumps({'error': f"Fichier PDF introuvable sur le serveur pour le groupe '{classification_group}'. Vérifiez que les PDF sont bien dans netlify/functions/flora_gallica_pdfs/."})
        }
    except ValueError as e:
        return { 'statusCode': 404, 'body': json.dumps({'error': str(e)}) }
    except Exception as e:
        return { 'statusCode': 500, 'body': json.dumps({'error': f"Une erreur interne est survenue: {str(e)}"}) }
