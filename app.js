...
// Génère un nom de fichier combinant l'espèce proposée et l'horodatage
function makeSpeciesTimestampName(species) {
  const d = new Date();
  const pad = n => n.toString().padStart(2, "0");
  const safe = species.replace(/[\\/:*?"<>|]/g, "-").trim();
  return `${safe} ${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())} ${pad(d.getHours())}h${pad(d.getMinutes())}.jpg`;
}
...
async function identifySingleImage(fileBlob, organ) {
  ...
  if (results) {
    ...
    if (first?.species?.scientificNameWithoutAuthor) {
      const sci = first.species.scientificNameWithoutAuthor;
      savePhotoLocally(fileBlob, makeSpeciesTimestampName(sci));
    }
