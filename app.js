...
const MAX_RESULTS = 5;                       // lignes tableau / fiches
/* ------------------------------------------------------------------ */

/* ----------- chargement taxref.json (latin ➜ CD_REF) -------------- */
let taxref = {};
...
async function identify(file){
  ...
  const res  = data.results.slice(0,MAX_RESULTS);
...
function buildTable(items){
  ...
  wrap.innerHTML="";
  const head=["Nom latin","Score (%)","INPN carte","INPN statut","Biodiv'AURA","OpenObs"];
